...
function sendEnquiry() {
	...
	if(valid) {
		...
		data:'name='+$("#name").val()+'&email='+$("#email").val()+'&phone='+$("#phone").val()+'&comment='+$("#comment").val(),
		type: "POST",
		success:function(data){
		$("#mail-status").html(data);
		$('input[type="text"],textarea').val('');
		$('input[type="email"],email').val('');
		$('input[type="text"],textarea').css('border-color','');
		$('input[type="email"],email').css('border-color','');
		},
		error:function (){}
		});
	}
...
formData.append('act','save');
	...
	if(valid) {
		jQuery.ajax({
		url: "enquirysubmit3.php",
		//data: FormData,
			data:'name='+$("#name").val()+'&Position='+$("#Position").val()+'&email='+$("#email").val()+'&phone='+$("#phone").val()+'&cv='+$("#cv").val(),
		type: "POST",
		contentType: false,
		cache: false,
		processData:false,
...
//input field contact number validation
...
function validateContact() {
	...
	if(!$("#name").val().match(/^[a-zA-Z\s]+$/)) {
		$("#name-info").html("(Only alphabets )");
		$("#name").css('border-color','#f20418');
		valid = false;
	}
	if(!$("#name").val()) {
	...
	if(!$("#comment").val()) {
		...
		$("#comment").css('border-color','#f20418');
		valid = false;
	}
            ...
            if ($(".inspiro-slider").length > 0) {
                $(".inspiro-slider").each(function () {
                    var elem = $(this),
                        ...
                        carouselAutoplayTimeout = elem.attr('data-autoplay-timeout') || 1000,
